...
| **GitLab** | AFTER local UI pass: `./scripts/publish.sh` (conventions check + first push) → pipeline must be green (see `.gitlab-ci.yml.example` stages: conventions → lint → typecheck → test → build → e2e → audit + `agent-evals` on config changes + triage on failure) → open MR: title `[PROJ-123][Type] Summary` + label `Jira::Story|Bug|Task`, template `templates/gitlab/merge_request_template.md`. Require: green pipeline, 1 approval, no unresolved Required threads, no secrets |
